...
impl AlgoStrategy for StrategyA {
    ...
    fn generate_algo_orders(
        ...
        let _state = state.instruments.instruments(&InstrumentFilter::None).next().unwrap();
        //let orders = Vec::new();
        //let data = state.data.clone();
        //println!("StrategyA {:?}", data);

        for inotify in state.instruments.instruments(&InstrumentFilter::None) {
            let market_data = inotify.data.market_data.clone();
            let price = market_data.price();
            if price.is_some() {
                println!("StrategyA {:?} {:?}", inotify.instrument.name_exchange.name(), market_data.price().unwrap());
